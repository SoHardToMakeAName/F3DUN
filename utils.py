...
def get_model_by_name(name, args):
    module = import_module("models." + name.upper())
    return module.make_model(args)
def get_distiller_by_name(name, args):
    module = import_module("distillers." + name.upper())
    return module.make_distiller(args)

def adapt_state_dict(state_dict, dataparallel=False):

    new_state_dcit = OrderedDict()
    for k, v in state_dict.items():
        if dataparallel is False and 'module' in k:
            name = k[7:]
        elif dataparallel and 'module' not in k:
            name = 'module.' + k
        else:
            name = k
