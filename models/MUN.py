...
class Block2D(nn.Module):
    def __init__(self, wn, n_feats) -> None:
        super().__init__()
...
class Block2Dv2(nn.Module):
    def __init__(self, wn, in_feats, out_feats):
        super(Block2Dv2, self).__init__()
        self.relu = nn.ReLU(inplace=True)
        
        self.fusion = wn(nn.Conv2d(in_feats, out_feats, kernel_size=(1, 1), stride=1))
        self.conv1 = wn(nn.Conv2d(out_feats, out_feats, kernel_size=(3, 3), stride=1, padding=(1, 1)))
        self.conv2 = wn(nn.Conv2d(out_feats, out_feats, kernel_size=(3, 3), stride=1, padding=(1, 1)))
...
class _3D2DModel(nn.Module):
    def __init__(self, args):
        super(_3D2DModel, self).__init__()

        scale = args.n_scale
        n_colors = args.n_colors
        n_feats = args.n_feats
        n_threeUint = args.n_threeUnit
        n_twoUint = args.n_twoUnit
        ...
        self.twoUnits = nn.ModuleList([Block2D(wn, n_feats)] + [Block2Dv2(wn, 2*n_feats, n_feats) for _ in range(n_twoUint)])
    ...
    def forward(self, x):
        ...
        for m in self.threeUnits:
            fea, _ = _to_4d_tensor(res)
            feas_3D.append(fea)
            res = m(res)
        x = res + x
        x, depth = _to_4d_tensor(x)
        res = x
        for i in range(len(self.twoUnits)):
            m = self.twoUnits[i]
            if i == 0:
                # print(res.shape)
                res = m(res)
